...
export function PlanRecommendations({ recommendations, isLoading = false }: PlanRecommendationsProps) {
    ...
    return (
      <div className="bg-gray-50 rounded-lg border border-gray-200 p-6">
        <div className="text-center py-8">
          ...
          <h3 className="mt-4 text-sm font-medium text-gray-900">No recommendations yet</h3>
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-6">
      ...
      <div>
        <h4 className="text-sm font-medium text-gray-700 mb-2">Proposed Title</h4>
      ...
      <div>
        <h4 className="text-sm font-medium text-gray-700 mb-2">Proposed Description</h4>
      ...
      <div>
        <h4 className="text-sm font-medium text-gray-700 mb-2">Key Learning Moments</h4>
        <ul className="space-y-2">
          {recommendations.keyLearningMoments.map((moment, index) => (
            <li key={index} className="flex items-start">
              <span className="flex-shrink-0 h-6 w-6 flex items-center justify-center rounded-full bg-primary/10 text-primary text-xs font-medium mr-3">
                {index + 1}
              </span>
              <span className="text-sm text-gray-900 pt-0.5">{moment}</span>
            </li>
          ))}
        </ul>
      </div>
