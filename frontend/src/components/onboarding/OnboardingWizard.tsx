            ...
            <div className="flex items-center justify-between mb-4">
              {steps.map((step, index) => (
                <div key={step.id} className="flex items-center flex-1">
                  <div
                    className={`flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-full transition-all duration-300 text-sm sm:text-base ${
          ...
          <div className="mt-6 sm:mt-8 flex justify-between gap-4">
            <button
              ...
              className={`px-4 sm:px-6 py-2 rounded-lg font-medium transition-colors ${
                isFirstStep
                  ? 'invisible'
                  : 'text-gray-700 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-400'
              }`}
            >
              Back
            </button>
